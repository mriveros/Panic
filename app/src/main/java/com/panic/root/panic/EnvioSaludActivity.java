...
public class EnvioSaludActivity extends Activity {
...
public boolean verificarDatos(String latitud, String longitud, String precision){
...
private class SendData extends AsyncTask {
	...
	protected void onPreExecute() {
		super.onPreExecute();
		pDialog = new ProgressDialog(EnvioSaludActivity.this);
		pDialog.setMessage("Enviando Salud..por favor espere");
	 ...
	 private void Datasend(){
	    ...
	    HttpPost httppost = new HttpPost("http://192.168.0.10/aguara/datareceptor_salud.php");
	    try {
	...
	private void enviarSMS(String phone){
		try {
			if (ActivityCompat.checkSelfPermission(this, android.Manifest.permission.SEND_SMS) != PackageManager.PERMISSION_GRANTED ) {
				...
				ActivityCompat.requestPermissions(this, new String[]{android.Manifest.permission.SEND_SMS, Manifest.permission.SEND_SMS}, 225);

			}
			SmsManager sms = SmsManager.getDefault();
			sms.sendTextMessage(phone,null,"Mensaje de Salud recibido. Lat: "+getIntent().getStringExtra("latitud")
					+" Long: "+getIntent().getStringExtra("longitud"),null,null);
			Toast.makeText(getApplicationContext(), "Mensaje de Salud enviado.", Toast.LENGTH_LONG).show();
		}
